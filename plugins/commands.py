...
async def startcmdssss_by_dkbotz(c, m:Message):
    ...
    NEW_USER_REPLY_MARKUP = [
                [
                    ...
                    InlineKeyboardButton("Connect To Anlinks🛠️", callback_data="dkbotz_balance")
                ]
            ]
...
async def balance_command_by_dkbotz(c, m: Message):
    try:
        ...
        if vld is not True:
            return await m.reply_text(f"Add API Key")
        ...
        if resp['status'] == 1:
            ...
            await m.reply_text(BALANCE_TEXT.format(username=username, pbalance=pbalance, rbalance=rbalance, tbalance=tbalance))
        if resp['status'] == 2:  
            await m.reply_text(f"Your Account in Pending")
        if resp['status'] == 3:
